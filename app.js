        ...
        let projectCounter = parseInt(localStorage.getItem('projectCounter') || '1');
        ...
        function initFiltersUI() {
          ...
          if (selectAllStatus) {
            selectAllStatus.addEventListener('change', () => {
              const statusCheckboxes = document.querySelectorAll('.dropdown-panel input[type="checkbox"][data-filter="status"]');
              const allChecked = selectAllStatus.checked;
              statusCheckboxes.forEach(cb => {
                cb.checked = allChecked;
                if (allChecked) { filterState.statuses.add(cb.value); }
                else { filterState.statuses.delete(cb.value); }
              });
              updateFilterBadges();
        ...
        function toggleSet(setObj, val, on) {
            if (on) setObj.add(val); else setObj.delete(val);
        }
        ...
        function renderActiveFilterChips() {
            ...
            // Search chip
            if (filterState.search) addChip('Search', filterState.search, () => { 
                filterState.search = ""; 
            ...
            // Status chips
            filterState.statuses.forEach(v => addChip('Status', v, () => {
                filterState.statuses.delete(v);
                const cb = document.querySelector(`input[type="checkbox"][data-filter="status"][value="${v}"]`);
                if (cb) cb.checked = false;
                updateFilterBadges(); renderAfterFilterChange();
            }));

            // Priority chips
            filterState.priorities.forEach(v => addChip('Priority', v, () => {
                filterState.priorities.delete(v);
                const cb = document.querySelector(`input[type="checkbox"][data-filter="priority"][value="${v}"]`);
                if (cb) cb.checked = false;
                updateFilterBadges(); renderAfterFilterChange();
            }));
            ...
            filterState.projects.forEach(pid => {
                ...
                addChip('Project', proj ? proj.name : pid, () => {
                    ...
                    const cb = document.querySelector(`input[type="checkbox"][data-filter="project"][value="${pid}"]`);
                    if (cb) cb.checked = false;
                    updateFilterBadges(); renderAfterFilterChange();
                });
            });
            ...
            // Date chip
            if (filterState.date) addChip('Date', filterState.date, () => {
                filterState.date = ""; 
            ...
            return tasks.filter(task => {
                ...
                if (dateFilter) {
                    ...
                    switch (dateFilter) {
                        ...
                        case 'this-month':
                            dOK = tDate.getMonth() === today.getMonth() && tDate.getFullYear() === today.getFullYear();
                            break;
                    }
                }
        ...
        function init() {
            // Initialize with demo data if there are no projects/tasks saved
            if (projects.length === 0) {
                projects = [
                    {
                        id: projectCounter++,
        ...
        function renderDashboard() {
            ...
            container.innerHTML = '<h3 style="margin-bottom: 16px; color: var(--text-secondary);">Recent Projects</h3>';
                ...
                <div class="project-card">
                    ...
                    <div class="project-meta">
                        <div class="project-stats">
                            ...
                            <span>✅ ${tasks.filter(t => t.projectId === project.id && t.status === 'done').length} done</span>
                        </div>
                ...
                rows.sort((a, b) => {
                    ...
                    switch (currentSort.column) {
                        ...
                        case 'status': {
                            ...
                            aVal = order[a.status] ?? 0; bVal = order[b.status] ?? 0; break;
                        }
                        case 'priority': {
        ...
        function updateSelectDisplay(selectId) {
            ...
            if (selected.length === 0 || hasAll) {
                if (selectId === 'filter-status') display.textContent = 'All Statuses ▼';
                else if (selectId === 'filter-priority') display.textContent = 'All Priorities ▼';
                else if (selectId === 'filter-project') display.textContent = 'All Projects ▼';
        ...
        function renderProjects() {
            ...
            if (projects.length === 0) {
                container.innerHTML = '<div class="empty-state"><h3>No projects yet</h3><p>Create your first project</p></div>';
                return;
            }
        ...
        function renderTasks() {
            ...
            source.forEach(t => { if (byStatus[t.status]) byStatus[t.status].push(t); });
            ...
            ['todo','progress','review','done'].forEach(status => {
                ...
                wrap.innerHTML = byStatus[status].map(task => {
        ...
        function openTaskDetails(taskId){
          ...
          // Status pill
          const statusLabels = { todo: 'To Do', progress: 'In Progress', review: 'Review', done: 'Done' };
          document.querySelector('.status-dot').className = `status-dot ${task.status}`;
          document.querySelector('.status-text').textContent = statusLabels[task.status];
        ...
        function openProjectModal() {
            ...
            document.querySelector('#project-form input[name="startDate"]').value = new Date().toISOString().split('T')[0];
            // Re-initialize date pickers for the modal
            setTimeout(() => {
                const dateInputs = document.querySelectorAll('#project-modal input[type="text"][name*="Date"]');
        ...
        function openTaskModal() {
          ...
          document.querySelector('#task-modal .btn-primary').textContent = 'Create Task';
        ...
        function submitTaskForm(){
          ...
          const dueISO = looksLikeDMY(dueRawNorm)
            ...
            : (looksLikeISO(dueRawNorm) ? dueRawNorm : '');
        ...
        function handleStatusDropdown(e) {
            ...
            const activeDropdowns = document.querySelectorAll('.status-dropdown.active');
        ...
        function renderCalendar() {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                dayTasks.slice(0, maxVisible).forEach(task => {
                    tasksHTML += `
                        <div class="calendar-task priority-${task.priority} ${task.status === 'done' ? 'done' : ''}" 
                             onclick="openTaskDetails(${task.id}); event.stopPropagation();">
                            ${task.title}
                        </div>
                    `;
                });
                ...
                if (dayTasks.length > maxVisible) {
                    tasksHTML += `<div class="calendar-more">+${dayTasks.length - maxVisible} more</div>`;
                }
        ...
        function renderProjectBars() {
            ...
            projects.forEach((project, projectIndex) => {
                ...
                if (startDate <= monthEnd && endDate >= monthStart) {
                    ...
                    console.log('Current month days:', currentMonthDays.map(d => ({day: d.day, index: d.index})));
                    ...
                    while (currentIndex <= endIndex) {
                        ...
                        bar.style.top = (baseTop + taskSpace + (projectIndex * (projectHeight + 2))) + 'px';
        ...
        function showDayTasks(dateStr) {
            ...
            if (dayTasks.length === 0 && dayProjects.length === 0) {
                ...
                const confirmCreate = confirm(`No tasks or projects for ${formatDate(dateStr)}. Create a new task?`);
                    if (confirmCreate) {
                        openTaskModal();
            ...
            const detailsHTML = `
                <div class="project-details-header">
                    ...
                    <div class="project-details-description">
                        <textarea class="editable-description" onchange="updateProjectField(${projectId}, 'description', this.value)">${project.description || 'No description provided for this epic.'}</textarea>
                    </div>
                    <div class="project-timeline">
                        <div class="timeline-item">
                            <div class="timeline-label">Start Date</div>
                            <input type="date" class="editable-date" value="${project.startDate}" onchange="updateProjectField(${projectId}, 'startDate', this.value)">
                        </div>
                        <div class="timeline-item">
                            <div class="timeline-label">End Date</div>
                            <input type="date" class="editable-date" value="${project.endDate || ''}" onchange="updateProjectField(${projectId}, 'endDate', this.value)">
                        </div>
                        <div class="timeline-item">
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-value">${formatDate(project.createdAt?.split('T')[0])}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="project-tasks-section">
                    <div class="section-header">
                        <div class="section-title">Tasks (${projectTasks.length})</div>
                            ...
                            : projectTasks.map(task => `
                                <div class="project-task-item" onclick="openTaskDetails(${task.id})">
                                    <div class="project-task-info">
                                        ...
                                        <div class="project-task-meta">Due: ${formatDate(task.dueDate)} • Priority: ${task.priority}</div>
                                    </div>
        ...
        function openTaskModalForProject(projectId) {
            ...
            document.querySelector('#task-form select[name="projectId"]').value = projectId;
        }
        ...
        function migrateDatesToISO(){
          ...
          projects.forEach(p => {
            if (p.startDate && looksLikeDMY(p.startDate)) { p.startDate = toISOFromDMY(p.startDate); touched = true; }
            if (p.endDate   && looksLikeDMY(p.endDate))   { p.endDate   = toISOFromDMY(p.endDate);   touched = true; }
          });
